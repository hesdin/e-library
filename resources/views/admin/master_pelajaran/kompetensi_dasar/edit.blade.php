            ...
            <div class="row">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Edit Kompetensi Dasar</h3>
                    </div>
                            ...
                            <div class="py-5">
                                <form class="form-data">
                                        ...
                                        <div class="mb-10">
                                            ...
                                            <select class="form-select form-control" id="mata_pelajaran_id" name="mata_pelajaran_id"
                                                ...
                                                @foreach ($mapel as $val)
                                                    <option value="{{ $val->id }}" {{ $data->mata_pelajaran_id == $val->id ? 'selected' : '' }}>
                                                        {{ $val->mata_pelajaran }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="d-flex justify-content-center gap-5">
                                        <button type="submit" class="btn btn-primary btn-sm btn-submit d-flex align-items-center">
                                            <i class="bi bi-file-earmark-diff"></i> Update
                                        </button>
                                        <button type="reset" id="side_form_close"
                                            class="btn mr-2 btn-light btn-cancel btn-sm d-flex align-items-center"
                                            style="background-color: #ea443e65; color: #EA443E">
                                            <i class="bi bi-trash-fill" style="color: #EA443E"></i> Batal
                                        </button>
                                    </div>
                                </form>
                            </div>
...
<script>
    ...
    .then(editor => {
        editor.setData(`{!! $data->kompetensi_inti !!}`);
    ...
    .then(editor => {
        editor.setData(`{!! $data->kompetensi_dasar !!}`);
        editor.model.document.on('change:data', () => {
    ...
    $(document).on('submit', ".form-data", function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route("kompetensi_dasar.update", $data->uuid) }}',
            ...
            success: function (response) {
                ...
                if (response.success == true) {
                    swal
                        .fire({
                            text: `Kompetensi Dasar berhasil diupdate`,
                            icon: "success",
            ...
            complete: function () {
                ...
                $(".btn-submit").html('<i class="bi bi-file-earmark-diff"></i> Update');
            },
        });
    });
</script>
